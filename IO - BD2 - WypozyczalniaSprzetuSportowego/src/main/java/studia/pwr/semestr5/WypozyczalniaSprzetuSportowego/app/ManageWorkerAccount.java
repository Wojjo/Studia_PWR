...
public class ManageWorkerAccount {
	...
	public ManageWorkerAccount() {
	}

	// jeśli create jest true, tworzy nowe konto, jeśli false to edytuje
	// istniejące
	public ManageWorkerAccount(MainWindow mainWindow, boolean create) {
	...
	public ManageWorkerAccount(MainWindow mainWindow, boolean create, Person person) {
		this.mainWindow = mainWindow;
		this.create = create;
		this.person = person;
	...
	private void initComponents() {
		...
		dialogCreateAccount.setSize(new Dimension(700, 450));
		...
		labelRequiredFields.setBounds(40, 380, 450, 30);
	...
	private void createWorkerAccount() {
		...
		if (login.equals(""))
			...
			JOptionPane.showMessageDialog(dialogCreateAccount, "Pole pensja nie moze byc pusta");
			...
			oracle.db_createWorker(mainWindow.getArrayListWorkers().size()+1,mainWindow.getArrayListAddresses().size()+1, mainWindow.getArrayListAddresses().size()+1, new Date(), salary2);
			...
			mainWindow.getArrayListPeople()
					.add(new Person(mainWindow.getArrayListAddresses().size() + 1, name, last_name, birth_date,
							phone_number, mainWindow.getArrayListAddresses().size() + 1, login, password, security_question,
							security_answer));
			mainWindow.getArrayListWorkers().add(new Worker(mainWindow.getArrayListWorkers().size() + 1, new Date(), salary2,
					0,  mainWindow.getArrayListPeople().size() + 1, false));
			dialogCreateAccount.dispose();
		}
	}
